        ...
        static void ListAll(SoftUniEntities context)
        {
            ...
            while (true)
            {
                ...
                foreach (var p in projects.Skip(pageSize * page).Take(pageSize))
                {
                    ...
                    if ((page * pageSize + pointer) < projects.Count) { current++; }
                ...
                switch (key.Key.ToString())
                {
                    case "UpArrow":
                        ...
                        else if (page > 0) { page--; pointer = 14; }
                        break;
                    case "DownArrow":
                        if ((page * pageSize + pointer) == projects.Count) { continue; }
                        else if (pointer < pageSize) { pointer++; }
        ...
        static void ShowDetails(SoftUniEntities context, int projectId)
        {
            ...
            while (true)
            {
                ...
                switch (key.Key.ToString())
                {
                    case "UpArrow":
                        if (pointer > 1) { pointer--; }
                        else if (page > 0) { page--; pointer = pageSize; }
                        break;
                    case "DownArrow":
